...
###### 1、Spark作业提交流程？
    ...
    5、每执行到一个 Action行动算子，就会创建一个 Job。Job 会提交给 DAGScheduler。
...
###### 5、使用Spark中遇到过哪些问题？如何解决的？
    数据倾斜
      - 按照特殊方法重新shuffle，按找key进行repartition
    内存溢出
      - 优化代码，对常用的数据进行cache，可以缓存到硬盘上
      - 增加内存大小
...
###### 9、SparkOnYarn作业执行流程？yarn-client和yarn-cluster的区别？
    ...
    在yarn-cluster模式下，Spark Driver运行在YARN集群中的一个ApplicationMaster节点上，而Executor同样运行在集群节点上。
    在这种模式下，ApplicationMaster负责与YARN ResourceManager交互，申请和释放资源，并且协调Executor的执行。
    无需担心Driver进程的内存和网络问题，但是无法直接监控和调试Driver进程。
    在Yarn-client模式下，Driver运行在Client上，通过AM向RM获取资源。这种模式下，客户端需要足够的内存来运行Driver进程，
    也需要稳定的网络连接，但是可以方便地对Driver进程进行监控和调试
...
###### 11、Spark中RDD如何通过记录更新的方式容错？
    ...
    checkpoint机制——数据检查点
    记录更新机制（在Saprk中对应Lineage机制）：它记录了每个RDD的依赖关系，以便在RDD数据丢失时能够恢复计算过程。
...
###### 16、Spark中persist()和cache()的区别？
    ...
    cache()方法是一种简化版的persist()方法，它默认将数据缓存在内存中
...
###### 52、Spark优化之数据本地性？
    ...
    分区策略：Spark将数据划分为不同的分区，每个分区都是独立的数据块。为了实现数据本地性，可以使用分区策略，将数据划分为与集群节点数相同的分区，使得每个节点都能够处理其本地分区的数据。这样就可以最大程度地利用数据本地性，减少数据的网络传输。
    缓存数据：可以使用cache()或persist()方法将数据缓存在内存中，以便在多个计算阶段中重复使用。这样可以减少重复的计算和数据的网络传输，提高计算效率。
    数据分片：将数据划分为多个小块，并将这些小块分散在不同的节点上。这样可以使得每个节点都能够处理其本地分片的数据，减少数据的网络传输。
    任务调度：Spark的任务调度器会将计算任务尽可能地分配给数据所在的节点，以实现数据本地性。可以使用spark.default.parallelism和spark.locality.wait等配置参数来优化任务调度策略，提高数据本地性。
...
###### [55）、IV,WOE计算？]()
    ...
    2. WOE的全称是“Weight of Evidence”，即证据权重。WOE是对原始自变量的一种编码形式。
    要对一个变量进行WOE编码，需要首先把这个变量进行分组处理（也叫离散化、分箱等等，说的都是一个意思）。
    
    feature在label中分布不足,或导致计算结果不准确,部分IV值会很大
    加入填充值后,整体IV值降低到合理区间
    ...
    3. AUC（Area Under Curve）被定义为ROC曲线下与坐标轴围成的面积，显然这个面积的数值不会大于1。
    又由于ROC曲线一般都处于y=x这条直线的上方，所以AUC的取值范围在0.5和1之间。
    AUC越接近1.0，检测方法真实性越高;
    等于0.5时，则真实性最低，无应用价值。
    
    4. KS（Kolmogorov-Smirnov）统计量由两位苏联数学家A.N. Kolmogorov和N.V. Smirnov提出。在风控中，KS常用于评估模型区分度。区分度越大，说明模型的风险排序能力（ranking ability）越强。
